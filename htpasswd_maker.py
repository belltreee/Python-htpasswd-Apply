...
layout_2 = sg.Frame('InPut ID/PW', [
    ...
    [sg.Button("Run", font=("Meiryo", 10), size=(5, 1), key="btn_run"),
     sg.Button("Exit", font=("Meiryo", 10), size=(5, 1), key="btn_exit", button_color=("#43576b")),
                    ...
                    tooltip='登録したいID/PWを入力してね！')

layout_3 = sg.Frame('Redirect', [
    [sg.Multiline(size=(83, 10), font=("Meiryo", 10), border_width=2, key='-Ridai-', background_color=("#6d86a1"),
                  text_color=("#edf9ff"))],
],
                    relief=sg.RELIEF_SUNKEN, font=("Meiryo", 10, 'bold'), title_color='NAVY')
...
choice = sg.popup_yes_no('.htpasswdファイルを新規に作成しますか？ \n [No]=既存ファイルへ追記')  # ★
...
if choice == 'Yes':
    ...
    if not os.path.isfile(path_file):
        sg.Popup(".htpasswdファイルがありません。 \n.htpasswdファイルをexeファイルの有るフォルダに配置してから再度起動して下さい。")
        sys.exit(1)

    ##ID-LIST.txtファイルがない場合は空ファイル作成
    if not os.path.isfile(path_file_id):
        with open(path_file, 'r', encoding='UTF-8') as f:
            lines = f.readlines()  # ファイルの中身全て読み込み、結果を文字列のリスト化する。
            for line in lines:  # lineで一行ずつ読み込む
                pattern = "(.*):(.*)"  # 読み込んだ文字列をグルーﾌﾟ化のパターンをpatterne変数に入れる ( group(1)：1番目のグループ（最初の括弧内）、group(2)：2番目のグループ（2つ目の括弧内）)
                d = re.search(pattern, line)  # pattern変数に入っているパターンの形式から、line変数に入っている文字列を検索する。
                print(d.group(1))  # group1の値のみ抽出される
                ID_LIST_file = open(path_file_id, 'a', encoding='UTF-8')
                ID_LIST_file.write('"' + d.group(1) + '", ')
                ID_LIST_file.close()
    else:
        pass
...
while True:
    ...
    ##dict_IDの値を抽出し、ID-LISTファイルに追加する。
